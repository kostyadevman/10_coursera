...
def get_courses_list(course_count):
    ...
    coursera_soup = bs4.BeautifulSoup(coursera_response.content, 'lxml')
...
def get_course_info(course_url):
    ...
    title = coursera_soup.find('h1', class_='title display-3-text').text
    start_date = coursera_soup.find(
        'div',
        class_='startdate rc-StartDateString caption-text'
    ).text.replace('Started', '').replace('Starts', '')
    language = coursera_soup.find('div', class_='rc-Language').contents[1]
    ...
    if user_rating_tag:
        user_rating = user_rating_tag.contents[0].text.replace('stars', '')
    else:
        user_rating = '-'
...
if __name__ == '__main__':
    ...
    output_courses_info_to_xlsx('1.xlsx', courses_info)